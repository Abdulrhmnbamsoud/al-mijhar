  ...
  try {
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: `أنت رئيس تحرير محترف لبرنامج حواري استقصائي (مثل برنامج الليوان). مهمتك هي استخراج زاوية رئيسية للحلقة، وبناء "خمسة" (5) محاور (فصول) دقيقة جداً بناءً على المصادر المقدمة. 
          مهم جداً:
          1. كل محور يجب أن يتشعب إلى قضايا وتفاصيل متعددة (لا تكتفِ بسؤال واحد للمحور، بل استخرج 4 إلى 5 أسئلة رئيسية في كل محور لتغطية أكثر من شغلة وزاوية داخل نفس المحور).
          2. الأسئلة يجب أن تكون حادة، استقصائية، ولا تقبل الإجابات الدبلوماسية.
          3. تجاهل أي معلومات أو مصادر تتحدث عن أشخاص آخرين يحملون أسماء مشابهة، ركز فقط على الضيف المستهدف بصفته ومجاله. 
